...
# Целевой блок диплинка (?start=...), на который нужно попасть после проверки
# подписки — если юзер пришёл по ссылке на конкретный раздел, но ещё не подписан.
PENDING_DEEPLINK: dict[int, str] = {}
